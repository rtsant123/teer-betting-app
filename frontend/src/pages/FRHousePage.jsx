...
const FRHousePage = () => {
  ...
  const addNumber = () => {
    ...
    if (number < 0 || number > 9) {
      toast.error('Number must be between 0 and 9');
      return;
    }
  ...
  const getTotalPotentialPayout = () => {
    const payoutRate = houseData?.house?.fr_house_payout_rate || 8;
    // Only ONE number can win in a game, so show the maximum possible payout from a single bet
    const maxSinglePayout = selectedNumbers.reduce((max, item) => {
      const payout = item.amount * payoutRate;
      return payout > max ? payout : max;
    }, 0);
    return maxSinglePayout;
  };
  ...
  const placeBets = async () => {
    ...
    try {
      ...
      const betData = {
        ...
        fr_house: houseNumbers  // Changed to use new schema
      };

      console.log('Sending FR house bet data:', betData);
        ...
        <div className="max-w-lg mx-auto px-3 sm:px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="text-center flex-1">
              <h1 className="text-lg sm:text-xl font-semibold text-gray-800">FR House</h1>
        ...
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <div className="font-bold text-lg">FR House</div>
                <div className="text-orange-100 text-sm">First round house digit</div>
              </div>
            </div>
            <div className="text-right">
              <div className="font-bold text-xl">{payoutRate}x</div>
              <div className="text-orange-100 text-xs">Payout</div>
            </div>
          </div>
          
          <div className="mt-3 pt-3 border-t border-orange-400 flex items-center justify-between">
            <div className="flex items-center">
              <Clock className="w-4 h-4 mr-2" />
              <span className="text-sm">Closes in: {timeUntilClose}</span>
            </div>
            <div className="bg-orange-400 px-2 py-1 rounded text-xs font-semibold">
              LIVE
            </div>
          </div>
        </div>
        ...
        <div className="w-full bg-white rounded-lg shadow-sm">
          ...
          <div className="p-4">
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="2"
                    className="w-full h-12 px-3 text-center text-lg font-bold border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                  />
                </div>
                <div>
              ...
              <button 
                onClick={addNumber}
                className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 px-4 rounded-lg font-semibold transition-colors disabled:opacity-50 flex items-center justify-center"
